...
public class RouteKafkaConsumerIdEvent {
	...
	public List<RouteModel> getEventsForRoute(String routeId) {
		// Crie uma cópia da lista allEvents
		List<RouteModel> allEventsCopy = new ArrayList<>(allEvents);

		// Filtrar os eventos que correspondem ao routeId especificado na cópia
		List<RouteModel> filteredEvents = allEventsCopy.stream().filter(route -> route.getId().equals(routeId))
